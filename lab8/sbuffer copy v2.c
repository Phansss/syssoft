...
#define SBUFF_ERROR_NO_EXIT(fmt, cond,...) 									                                                    \
        do {if (cond) {                                                                                                                \
                tid = syscall(__NR_gettid);                                                                                     \
...
void *reader_thread(void *arg) {
    ...
    while(1) {
        if (pthread_rwlock_tryrdlock(_sbuffer_->rwlock_rw_writing) == EBUSY) {               // writer still busy; lock not acquired.
            ...
            result = pthread_barrier_wait(_sbuffer_->barrier_r_readerssync);                 // wait for other reader!
            if(result == 0) {    //  close every reader thread except 1
                SBUFF_DEBUGGER_PRINTF("Reader exiting'")                             
                pthread_exit(0);
            }
            SBUFF_DEBUGGER_PRINTF("Reader cleaning up %d sbuffer nodes'", sbuffer_size(_sbuffer_))
            while(sbuffer_size(_sbuffer_) > 0) {
                sbuffer_remove(_sbuffer_, &data, 1);
                (_callback_func_)(_callback_arg_);               
                count++;
            }
